...
class Scene:
    ...
    def __init__(self, robot=None, sceneID=0):
        ...
        self.op_dialog = [
            ...
            "Mopping Floor",
            "Wiping Table",
            "Turning on Tube Light",
            "Moving Chairs",
            "Turning off Tube Light",
            "Turning on Hall Light",
            "Turning off Hall Light",
            "Closing Curtains",
            "Opening Curtains",
            "Toggling Air Conditioning",
    ...
    def add_walker(self, id, x, y, yaw=0, v=0, scope=100):
        ...
        if (str(scene.info).find('unreachable') > -1):
            print('The current location is unreachable and cannot initialize NPCs')
        else:
            walker_list.append(
    ...
    def move_task_area(self, op_type, obj_id=0, release_pos=[247.0, 520.0, 100.0]):
        ...
        print("Current Position:", cur_pos, "Start:", self.op_dialog[op_type])
        if op_type == 11 or op_type == 12:
            return
        print('------------------moveTo_Area----------------------')
        if op_type < 8:
            if self.show_ui:
        ...
        if op_type == 16:
            ...
            if 820 <= obj_y <= 1200 and 240 <= obj_x <= 500:
                walk_v = [obj_x + 40, obj_y - 35, 130, 180, 0]
        if op_type == 17:
    ...
    def gen_obj(self, h=100):
        ...
        obj_list = [
            ...
            GrabSim_pb2.ObjectList.Object(x=370, y=1000, z=88, roll=0, pitch=0, yaw=90, type=1),
            GrabSim_pb2.ObjectList.Object(x=380, y=1000, z=88, roll=0, pitch=0, yaw=90, type=65),
            GrabSim_pb2.ObjectList.Object(x=380, y=1020, z=88, roll=0, pitch=0, yaw=90, type=42),
            GrabSim_pb2.ObjectList.Object(x=360, y=1020, z=88, roll=0, pitch=0, yaw=90, type=27),


            GrabSim_pb2.ObjectList.Object(x=-15, y=1050, z=40, roll=0, pitch=0, yaw=90, type=17),
    ...
    def op_task_execute(self, op_type, obj_id=0, release_pos=[247.0, 520.0, 100.0]):
        ...
        self.control_robot_action(0, 1, "Start " + self.op_dialog[op_type])
        if op_type < 8:
            if self.show_ui:
                self.get_obstacle_point(self.db, self.status, map_ratio=self.map_ratio)
    ...
    def get_obstacle_point(self, db, scene, map_ratio, update_info_count=0, is_nav=False):
        ...
        semantic_info_str=""
        semantic_info_str+=  f'Count pedestrian numbers：{walker_detect_count}'+"\n\n"
        semantic_info_str += f'Detect object count：{obj_detect_count}' + "\n\n"
        semantic_info_str += f'Update semantic information：{update_info_count}' + "\n\n"


        file_json_name = os.path.join(root_path, 'robowaiter/proto/objs.json')
        with open(file_json_name) as f:
            data = json.load(f)
        semantic_info_str += f'Stored semantic information：{len(data)}' + "\n"
    ...
    def updateMap(self, points):
        ...
        for obs in points:
            ...
            (x, y) = obs
            occupy_radius = self.navigator.planner.dyna_obs_radius
