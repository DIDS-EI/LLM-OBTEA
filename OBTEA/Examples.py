...
#
# In this example, placing the VacuumCup on the FrontDesk has a smaller cost compared to performing MoveTo(Table) followed by Put(Table, VacuumCup).
def SoftdrinkCost():
    actions=[
...
# In this example, placing the VacuumCup on the FrontDesk incurs a lower cost than performing MoveTo(Table) followed by Put(Table, VacuumCup).
def MakeCoffeeCost():
    actions=[
...
# todo: The most original example
def MoveBtoB_num ():
    actions=[]
    a = Action(name='a1')
    a.pre={1,4}
